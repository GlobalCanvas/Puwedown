...
//  Запускает веб-сервер как дочерний процесс при старте бота.
// ════════════════════════════════════════════════════════════════════════════
(function startWebServer() {
  const serverPath = path.resolve(__dirname, 'server.js');
  if (!fs.existsSync(serverPath)) {
    log('warn', '⚠️  server.js not found — web server NOT started');
    return;
  }

  function launch() {
    const child = spawn(process.execPath, [serverPath], {
      env:   process.env,          // передаём все переменные окружения (BOT_TOKEN и т.д.)
      stdio: 'inherit',            // логи сервера пишутся в тот же терминал
      detached: false,
    });

    log('info', `🌐 Web server started (PID ${child.pid})`);

    child.on('error', err => log('error', 'Web server error:', err.message));

    child.on('exit', (code, signal) => {
      log('warn', `Web server exited (code=${code}, signal=${signal}) — restarting in 3 s…`);
      setTimeout(launch, 3000);   // авто-перезапуск при падении
    });

    // Завершаем дочерний процесс вместе с ботом
