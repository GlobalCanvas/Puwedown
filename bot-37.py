...
# ── Одноразові токени для логіну через бота (спільний файл з webapp) ──
...
def create_login_token(uid: int) -> str:
    ...
    with _LT_LOCK:
        data = _lt_load()
        # чистимо прострочені та старі токени цього юзера
...
async def cmd_start(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    # /start webapp — генерируем одноразовую ссылку для входа на сайт
    args = ctx.args or []
...
"
        ...
        kb = InlineKeyboardMarkup([[
            InlineKeyboardButton("🔓 Открыть сайт", url=login_url)
        ]])
