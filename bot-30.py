...
def main():
    ...
    # ── Запуск веб-сервера (miniapp) на порту 80 ──
    import threading, sys
    def run_webapp():
        import importlib.util, pathlib
        webapp_path = pathlib.Path(__file__).parent / "webapp.py"
        spec = importlib.util.spec_from_file_location("webapp", webapp_path)
        mod = importlib.util.module_from_spec(spec)
        spec.loader.exec_module(mod)
        mod.cleanup_old_files()
        mod.app.run(host="0.0.0.0", port=80, debug=False, threaded=True)
