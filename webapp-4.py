...
# ── Login tokens (shared with bot via file) ──
...
def lt_consume(token: str):
    """Returns uid if token valid, else None. Deletes token after use."""
    with _LT_LOCK:
        data = lt_load()
...
class Handler(BaseHTTPRequestHandler):
    ...
    def do_GET(self):
        ...
        # /login/<token> — одноразовий логін з бота
        if path.startswith("/login/"):
            token = path[7:]
            uid = lt_consume(token)
            if not uid:
                body = b"<html><body><h2>&#128683; Ссылка недействительна или истекла.</h2><p>Запросите новую в боте: /start</p></body></html>"
                self.send_response(200)
                ...
                self.wfile.write(body)
                return
            # генеруємо сесійний токен і повертаємо HTML що авто-логінить
            u = db_get_user(uid)
            if not u:
                db_upsert_user(uid, "", "")
                u = db_get_user(uid)
